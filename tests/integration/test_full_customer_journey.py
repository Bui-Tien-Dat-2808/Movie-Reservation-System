...
@pytest.mark.asyncio
async def test_full_customer_journey_e2e(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    room = Room(
        name="E2E IMAX Hall",
        room_type=RoomType.IMAX,
        room_number=1,
        total_rows=5,
        total_cols=5,
        is_active=True,
    )
    db_session.add_all([movie, room])
    await db_session.flush()

    s1 = Seat(room_id=room.id, row_label="A", col_number=1, seat_type=SeatType.STANDARD, is_active=True)
    s2 = Seat(room_id=room.id, row_label="A", col_number=2, seat_type=SeatType.STANDARD, is_active=True)
    ...
    seat1 = ShowtimeSeat(showtime_id=showtime.id, seat_id=s1.id, status=SeatStatus.AVAILABLE)
    seat2 = ShowtimeSeat(showtime_id=showtime.id, seat_id=s2.id, status=SeatStatus.AVAILABLE)
    ...
    db_session.add_all([voucher, concession])
    ...
    # Step 4: Hold Seats
    hold_resp = await client.post(f"/api/v1/showtimes/{showtime.id}/hold", json={"seat_ids": [s1.id, s2.id]}, headers=headers)
    ...
    with patch("app.services.refund_service.RefundService._call_vnpay_refund_api", return_value={"vnp_ResponseCode": "99", "vnp_Message": "Sandbox Limit"}):
